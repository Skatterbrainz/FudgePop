	...
	foreach ($computer in $ComputerName) {
		...
		foreach ($className in $classes.keys) {
			...
			if (!$isLocal) {
				...
				try {
					$inv += Get-WmiObject -Query $query -ComputerName $cname -ErrorAction Stop | Select-Object $proplist | ConvertTo-Html -Fragment
				} catch {
				...
				try {
					$inv += Get-WmiObject -Query $query -ErrorAction Stop | Select-Object $proplist | ConvertTo-Html -Fragment
				} catch {
